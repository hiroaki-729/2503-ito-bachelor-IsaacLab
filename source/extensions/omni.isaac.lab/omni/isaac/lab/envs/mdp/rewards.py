...
def joint_vel_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    """
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]       # どの報酬関数でもここは同じ
    # a=asset.data._previous_joint_vel[:, asset_cfg.joint_ids]
    # b=asset.data.joint_acc[:, asset_cfg.joint_ids]
    # print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",asset.data.joint_pos)
    return torch.sum(torch.square(asset.data.joint_vel[:, asset_cfg.joint_ids]), dim=1) #各関節速さの2乗
...
def action_rate_l2(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    # print("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz",env.action_manager.action)
...
def energy_consumption(env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),gamma=0.05) -> torch.Tensor:
    ...
    # print("aaaaaaaaaaaaaaaaaaaaaaaaaaaa",pos)
    ...
    pr=printr.min()   #*(0.0001)*env.step_dt
